...
class GeolocatorOhos extends GeolocatorPlatform {
  ...
  Future<LocationPermission> checkPermission() async {
    print('ac check permission');
    try {
      // ignore: omit_local_variable_types
      final int permission = await _methodChannel
          .invokeMethod('checkPermission')
          .onError((error, stackTrace) {
        print(error);
        print(stackTrace);
      });
      print('checked, permission=$permission');
  ...
  Future<Position?> getLastKnownPosition({
    ...
    try {
      ...
      return positionMap != null ? OhosPosition.fromMap(positionMap) : null;
    } on PlatformException catch (e) {
      final error = _handlePlatformException(e);
  ...
  Future<Position> getCurrentPosition({
    ...
    try {
      ...
      print('position map =$positionMap');
      return OhosPosition.fromMap(positionMap);
    } on TimeoutException {
      final parameters = <String, dynamic>{
        'requestId': requestId,
      };
  ...
  Stream<Position> getPositionStream({
    ...
    _positionStream = positionStream
        .map<Position>((dynamic element) =>
            OhosPosition.fromMap(element.cast<String, dynamic>()))
        .handleError(
      (error) {
        if (error is PlatformException) {
          error = _handlePlatformException(error);
